...
def test_generic_funnel_schema_is_used():
    data = load_data_package()
    assert DATASET_CONFIG["funnel"]["required_columns"] == [
        "week", "visitors", "browse_users", "product_viewers",
        "cart_users", "checkout_users",
    ]
    assert "food_visitors" not in data["funnel"].columns
    assert "restaurant_viewers" not in data["funnel"].columns
    assert "menu_viewers" not in data["funnel"].columns
...
def test_funnel_stage_order_failure():
    data = load_data_package()
    data["funnel"].loc[0, "product_viewers"] = (
        data["funnel"].loc[0, "browse_users"] + 1
    )
    results = validate_data_package(data)
