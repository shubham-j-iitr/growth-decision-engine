...
funnel.csv
-----------
week,food_visitors,restaurant_viewers,menu_viewers,cart_users,checkout_users
...
def _normalise_dataset_schema(key: str, df: pd.DataFrame) -> pd.DataFrame:
    ...
    if key == "funnel" and "mau" in out.columns:
        return out[[
...
with tab_diagnosis:
    ...
    with funnel_cols[0]:
        food_engagement_fig = kpi_trend(
            funnel_analysis["weekly"],
            "food_engagement_rate",
            "Food Engagement Rate",
        )
        food_engagement_fig.update_yaxes(ticksuffix="%")
        st.plotly_chart(
            food_engagement_fig,
            use_container_width=True,
        )
